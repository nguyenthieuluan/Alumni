...
/* -----------------------------
    ...
    CRUMINA.InitBaseElem = function() {
        ...
        * Input Number Quantity
        ...
        $(".profile-settings-open").on('click',
            function() {
                $('.profile-settings-responsive').toggleClass('open');
                return false
            });

        $(".js-expanded-menu").on('click',
            function() {
        ...
        $(".js-open-responsive-menu").on('click',
            function() {
                $('.header-menu').toggleClass('open');
                return false
            });

        $(".js-close-responsive-menu").on('click',
            function() {
                $('.header-menu').removeClass('open');
                return false
            });
...
/* -----------------------------
    ...
    CRUMINA.Bootstrap = function() {
        ...
        if (date_select_field.length) {
            ...
            date_select_field.on('focus',
                function() {
                    $(this).closest('.form-group').addClass('is-focused');
                });
            date_select_field.on('apply.daterangepicker',
                function(ev, picker) {
                    $(this).val(picker.startDate.format('DD/MM/YYYY'));
                    $(this).closest('.form-group').addClass('is-focused');
                });
            date_select_field.on('hide.daterangepicker',
                function() {
                    if ('' === $(this).val()) {
 ...
 * ---------------------------*/
    ...
    CRUMINA.HeaderSearch = function() {
        ...
        //        options: [
        //            {
        //                image: 'theme/img/avatar30-sm.jpg',
        ...
        //                    (item.image
        //                        ? '<div class="author-thumb"><img src="' + escape(item.image) + '" alt="avatar"></div>'
        //                        : '') +
        //                    '<div class="notification-event">' +
        //                    (item.name
        //                        ? '<span class="h6 notification-friend"></a>' + escape(item.name) + '</span>'
        //                        : '') +
        //                    (item.message
        //                        ? '<span class="chat-message-item">' + escape(item.message) + '</span>'
        //                        : '') +
        //                    '</div>' +
        //                    (item.icon
        //                        ? '<span class="notification-icon"><svg class="' +
        //                        escape(item.icon) +
        //                        '"><use xlink:href="theme/svg-icons/sprites/icons.svg#' +
        //                        escape(item.icon) +
        //                        '"></use></svg></span>'
        //                        : '') +
        //                    '</div>';
...
/* -----------------------------
    ...
    CRUMINA.InitSlideCarousel = function() {
        //var initIterator = 0;
        ...
        ////swiper arrows
        //$('.btn-prev-without').on('click',
        //    function() {
        //        var sliderID = $(this).closest('.swiper-container').attr('id');
        //        swipers['swiper-' + sliderID].slidePrev();
        //    });

        //$('.btn-next-without').on('click',
        //    function() {
        //        var sliderID = $(this).closest('.swiper-container').attr('id');
        //        swipers['swiper-' + sliderID].slideNext();
        //    });
        ...
        //// Click on thumbs
        //$('.slider-slides .slides-item').on('click',
        //    function() {
...
/* -----------------------------
	...
	* ---------------------------*/
    ...
    CRUMINA.mediaPopups = function() {
        ...
        if (typeof $.fn.mediaelementplayer !== 'undefined') {
            $('#mediaplayer').mediaelementplayer({
                "features": [
                    'prevtrack', 'playpause', 'nexttrack', 'loop', 'shuffle', 'current', 'progress', 'duration',
                    'volume'
                ]
            });
        }
    ...
    CRUMINA.Scrollbar = function() {
        $('.mCustomScrollbar').perfectScrollbar({ wheelPropagation: false });

    };
CRUMINA.InitAll = function() {
    CRUMINA.InitBaseElem();
    CRUMINA.ProgressBar();
    CRUMINA.FixedHeader();
    CRUMINA.InitCounter();
    CRUMINA.Materialize();
    ...
    CRUMINA.Bootstrap();
    CRUMINA.HeaderSearch();
    CRUMINA.InitSlideCarousel();
    CRUMINA.IsotopeSort();
