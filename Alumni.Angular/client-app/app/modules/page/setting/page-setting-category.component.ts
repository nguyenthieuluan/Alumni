...
import { UserProfileServiceProxy, UserProfileDto, PageDetailDto, PageServiceProxy, PageCategory } from "@shared/service-proxies/service-proxies";
...
export class PageSettingCategoryComponent extends AppComponentBase implements OnInit, AfterViewInit {
    page : PageDetailDto;
    ...
    save() {
        if (this.isEdit) {
                    abp.ui.setBusy();
            this._remotePageService.updatePageCategory(this.pageCategory).subscribe(r => {
                this.setPage(r);
                this.editModal.hide();
                abp.ui.clearBusy();
                this.notify.success("Your information is successfuly saved.", "", { positionClass: 'toast-top-right' });
                    ...
                    abp.ui.setBusy();
            this._remotePageService.createPageCategory(this.pageCategory).subscribe(r => {
                this.setPage(r);
                this.editModal.hide();
                abp.ui.clearBusy();
                this.notify.success("Your information is successfuly saved.", "", { positionClass: 'toast-top-right' });
    ...
    deleteCategory(pc: PageCategory): void {
        ...
        abp.message.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    abp.ui.setBusy();
                    this._remotePageService.removePageCategory(pc)
                        .subscribe((r) => {
                            if (r) {
                                this.page.categories = this.page.categories.filter(x => x.code !== pc.code);
                                
                                abp.ui.clearBusy();
                                 abp.notify.info("Deleted category: " + pc.name, "", { positionClass: 'toast-top-right' });
                            }
