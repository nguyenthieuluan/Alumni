...
export class UserHeaderProfileComponent extends AppComponentBase implements OnInit {
    ...
    ngOnInit(): void {
        if (this._userService.activeUserProfile && this._userService.activeUserProfile.userId != this.appSession.user.id) {
            this._userProfileService.checkFriend(this._userService.activeUserProfile).subscribe(
                r => {
                    this.friendStatus = r;
    ...
    addFriend() {
        this._userProfileService.requestFriend(this._userService.activeUserProfile)
            .subscribe( r => {
                if (r)
                    this.friendStatus = 2;
            }
    ...
    unFriend() {
        this._userProfileService.unFriendFriend(this._userService.activeUserProfile).subscribe(r => {
            this.friendStatus = 0;
        });
    }
}